...
case $? in
  0)
	if [ "$UID" = "0" ] && [ ! -z "`cat $HDSTORE`" ] ; then
		...
		wait
		echo "Rereading partitions ..."
		color off
		for disk in `awk '$NF ~ /[h,s,p]d[a-z]/ {print $NF}' /proc/partitions | grep -v "[0-9]$"`
		do
		        /sbin/blockdev --rereadpt $disk
		done
		func_gather_disc_info
		exec >/dev/console 2>&1 </dev/console
...
none            "I do not want to install any bootloader " off 2> $bootloaderstore
[ -z "`head -n 1 $bootloaderstore`" ] && echo none > $bootloaderstore
...
bootloader=`head -n 1 $bootloaderstore | cut -c1-4 2>/dev/null`
