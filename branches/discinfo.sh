...
line=0
for check in `fdisk -l 2>/dev/null| grep ^\/dev | cut -c1-8 |sort -u` ; do
...
for dev in `awk '$4 ~/[h,s]d[a-z]$/ {print $4}' /proc/partitions 2>/dev/null` ; do

if [ "`cat /proc/ide/$dev/media 2>/dev/null`" = "disk" ] ; then
	echo "$dev `tr ' ' _ </proc/ide/$dev/model`" >> $disks_choice
	NUMHD=$[NUMHD+1]
elif [ -n "`cat /sys/block/$dev/removable 2>/dev/null`" ] ; then
...
fdisk -l 2>/dev/null| grep ^\/dev | awk '{print $1}' |sort -u |grep -v md > $PARTITIONS

sed -i -e '/^md/d' $disks_choice
...
for part in `fdisk -l 2>/dev/null| grep ^\/dev | awk '{print $1}' 2>/dev/null` ; do
...
grep "ext2\\|ext3\\|ext4\\|reiserfs\\|reiser4\\|xfs" $ALL_PARTITIONS > $LINUX_PARTITIONS
grep "ext2\\|ext3\\|ext4\\|reiserfs\\|reiser4\\|xfs" $ALL_PARTITIONS > $output
