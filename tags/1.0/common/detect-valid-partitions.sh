...
mkdir -p $TOP_DIR/partitions

for rem in `ls -1d $TOP_DIR/partitions/* > /dev/null 2>&1` ; do 
...
line=0
for check in `cat /var/tmp/disks` ; do
...
for part in `echo $partitions` ; do
	...
	mkdir -p $TOP_DIR/partitions/$part_mnt
	mount $part $TOP_DIR/partitions/$part_mnt > /dev/null 2>&1 || continue
