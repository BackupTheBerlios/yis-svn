...
ssh-keygen -P "" -t rsa1 -f /etc/ssh/ssh_host_key > /dev/null
echo 80
ssh-keygen -P "" -t rsa -f /etc/ssh/ssh_host_rsa_key > /dev/null
echo 90
ssh-keygen -P "" -t dsa -f /etc/ssh/ssh_host_dsa_key > /dev/null
echo 100
) | \
...
/usr/sbin/useradd -u 1000 -g 100 -s /bin/bash -m $user
...
rm -rf /var/tmp/* /tmp/* /yis /settings /none

for service in syslog-ng haldaemon kdmctrl cupsd dhclient ; do
	[ -f /etc/rc.d/init.d/$service ] && chkconfig --add $service >/dev/null 2>&1
done
