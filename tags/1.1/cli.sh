...
$DIALOG --backtitle "YOUR Operating System Installation Program"  --title Partitioning \
  ...
  --radiolist "Please choose the Harddisk (spacebar to select) to run cfdisk on, or just press enter to proceed.\n\nIt is highly recommended to reboot if you make changes to your hard disk before proceeding with the installation." 25 60 $LINES \
...
case $? in
  0)
	if [ "$UID" == "0" ] && [ ! -z "`cat $HDSTORE`" ] ; then
		...
		/usr/sbin/partprobe >/dev/null 2>&1 || /sbin/reboot
...
[ -z "`cat $PMENU 2>/dev/null`" ] && abort_on_error "No partitions with at least 2 GB found. Please run cfdisk in the installer!"

rm -f $PSTORE

$DIALOG --backtitle "YOUR Operating System Installation Program"  --title Partitioning \
--ok-label "Proceed With Installation" --no-cancel \
--radiolist "Press the SPACE key to select partition for : $1 ,\nor just press enter to proceed.\n
\nYou only have to choose a root partition.
...
for dev in `fdisk -l |grep ^\/dev | awk '{print $1}'` ; do
...
--ok-label "Proceed Installation" --no-cancel \
--radiolist "Please select which bootloader to install into hd0 ." 15 50 5 \
grub            "Default Yoper Bootloader	" off \
lilo            "Fallback bootloader            " off \
none            "I do not want to install any bootloader " on 2> $bootloader
...
if [ "$bootloader" != "none" ] ; then
	...
	[ "$bootloader" == "grub" ] && bline2="Note:\n\nDuring our tests it came to our attention that the installation of the grub-bootloader randomly fails. If grub installation fails it will fall back to lilo instead. "
else
...
cp_pid="`ps ax | grep "cp -a /KNOPPIX" | grep -v grep| awk '{print $1}'`" 
sleep 1
...
if [ "$bootloader" == "grub" ] ; then
	...
	$TOP_DIR/common/install-grub.sh > /dev/null || abort_on_error "Bootloader installation failed"
fi
